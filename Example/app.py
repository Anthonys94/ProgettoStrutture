...
UtentiPrevisti = ['Master', 'Amministratore', 'Operatore', 'Leader', 'Utente Pro', 'Utente Semplice']
app = Flask(__name__)
...
'''
TO DO ALL: METTERE I RIFERIMENTI AD HOME
Opetatore vede solo la lista di materiali, strutture e prove
Gestire la concorrenza
mettere il controllo del .pdf
'''
    ...
    if login.count() == 1:
        ...
        if login[0]["ruolo"] == 'Utente Semplice' and Scaduto(login[0]["dataInserimento"].date().strftime("%Y-%m-%d")):
            ...
            if login[0]["ruolo"] == 'Amministratore':
                ...
                resp = make_response(render_template("Leader/homeLeader.html"))
            elif login[0]["ruolo"] == 'Operatore':
                resp = make_response(render_template("Operatore/HomeOperatore.html"))
            elif login[0]["ruolo"] in ['Utente Semplice', 'Utente Pro']:
                resp = make_response(render_template("Utente/HomeUser.html"))
...
@app.route('/Ricerca_ad.html', methods=['GET'])
def getRicerca():
...
def main_Documento():
    ...
    if ruolo in ['Leader', 'Amministratore', 'Operatore']:
        return AnnullaTuttoVaiLogIn(username, ruolo, data, 'ErroreInserimento.html')
    else:
    ...
    if ([username, ruolo] not in regUsers) or (ruolo == 'Utente Semplice' and Scaduto(data)):
        ...
        return render_template('ErroreInserimento.html')
    '''
...
def getCertificati():
    ...
    for element in certificati:
        ob = {}
        ob['_id'] = (element['_id']['NumeroPratica']) if element['_id']['Lettera'] == "unique" else  (element['_id']['NumeroPratica']+ '/' + element['_id']['Lettera'])
...
def newCertificato():
    ...
    while (esito_cert and index < numeroDocumenti):
        certificato = request.files['Certificato' + str(index)]
        certificato.filename = request.form['TextPratica'] + '_' + request.form['Lettera' + str(index)].upper() + '.pdf'
        certificato.filename = secure_filename(certificato.filename)
        esito_cert = gestore.buildCertificato(request.form['TextPratica'], request.form['Lettera' + str(index)].upper(),
                                              request.form['Struttura' + str(index)],
                                              request.form['Materiale' + str(index)],
...
def get_single_materiale():
    struttura = request.args.get('Materiale')
    gestore = GestoreForWrite()
    element = gestore.get_single_materiale(struttura)
    ob = {}
    ob['esito'] = True
    ob['id'] = element[0]["_id"]
...
def upload_struttura():
    gestore = GestoreForWrite()
    esito = gestore.buildTipologiaStruttura(request.form['TipologiadiStruttura'], request.form['Descrizionestruttura'])
    ob = {}
    ob['check'] = esito
    return jsonify(ob)
    '''
...
def checkUtente():
    gestore = GestoreForWrite()
    ob = {}
    esito = gestore.checkEsistenzaUtente(request.args.get('CF'))
    ob['esito'] = esito
    return jsonify(ob)
...
def checkUtente_attivo():
    ...
    if us.count() > 0 and not Scaduto(us[0]['dataInserimento'], us[0]['flag']):
        ob['esito'] = True
    else:
        ob['esito'] = False
    return jsonify(ob)
...
def newUtente():
    ...
    esito = gestore.addUtente(request.form['Nome'], request.form['Cognome'], request.form['Provincia'],
                              request.form['Comune'], request.form['bday'], request.form['CodiceFiscale'].upper(),
                              request.form['Password'], request.form['Ruolo'], request.form['Scadenza'] == "true",
...
def getUtenti():
    ...
    for element in utenti:
        ...
        if not Scaduto(element['dataInserimento'], element['flag']):
            Attivi.append(ob)
        else:
            Scaduti.append(ob)
...
def check_esistenza_utente_inattivo():
    ...
    us = gestore.getSingleUser(request.args.get('CF').upper())
    print(us[0])
    if us.count() > 0 and Scaduto(us[0]['dataInserimento'], us[0]['flag']):
        ob['esito'] = True
    else:
        ob['esito'] = False
    return jsonify(ob)
